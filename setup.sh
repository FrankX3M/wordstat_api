...
"""
handlers/export.py - ИСПРАВЛЕННАЯ ВЕРСИЯ v3.5
Исправлена обработка дат для экспортов pages_in_search и page_events
"""
...
async def select_export_type(callback: CallbackQuery, state: FSMContext):
    ...
    # ✅ ИСПРАВЛЕНИЕ: Для типов без выбора дат сразу устанавливаем текущую дату
    if export_type in ["pages_in_search", "page_events"]:
        today = datetime.now().date().isoformat()
        await state.update_data(date_from=today, date_to=today)
        
        logger.info(f"   Setting default dates for {export_type}: {today}")
        
        type_descriptions = {
            "pages_in_search": {
                "name": "🔗 Страницы в поиске",
                "desc": "Список URL страниц, которые находятся в индексе Yandex. Полезно для аудита индексации."
            },
            "page_events": {
                "name": "📋 События со страницами",
                "desc": "История добавления и удаления страниц из поиска. Помогает отследить изменения в индексации."
            }
        }
        
        type_info = type_descriptions.get(export_type, {
        ...
        await callback.message.edit_text(
            ...
            f"━━━━━━━━━━━━━━━━━━━━\n\n"
            f"⚠️ <b>Важно:</b> Этот тип экспорта не требует выбора периода дат.\n"
            f"Будут выгружены актуальные данные на текущий момент.\n\n"
            f"Нажмите <b>\"▶️ Продолжить\"</b> для выбора параметров экспорта.",
...
# ============================================================================
# 3. КНОПКА "ПРОДОЛЖИТЬ" (для pages_in_search и page_events)
# ============================================================================

@router.callback_query(F.data == "continue_export")
async def continue_export_handler(callback: CallbackQuery, state: FSMContext):
    """
    ✅ ИСПРАВЛЕНО: Продолжить после информационного сообщения
    Даты уже установлены в select_export_type
    """
    await callback.answer()
    ...
    logger.info(f"👤 User {user_id} continuing {export_type} export")
    logger.info(f"   Dates already set: {date_from} to {date_to}")
    
    # Переходим сразу к выбору устройства
    await callback.message.edit_text(
    ...
    logger.info("✅ Device selection displayed")


# ============================================================================
# 4. ВЫБОР ПЕРИОДА ДАТ (для обычных экспортов)
# ============================================================================
...
async def process_date_from(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        f"Например: <code>2024-12-22</code>",
        reply_markup=get_back_button("back_to_date_select")
    )
...
# ============================================================================
# 7. ВЫБОР ФОРМАТА И ЗАПУСК ЭКСПОРТА
# ============================================================================
...
async def select_export_format(callback: CallbackQuery, state: FSMContext):
    ...
    logger.info(f"  export_type: {export_type} {'✅' if export_type else '❌'}")
    logger.info(f"  device_type: {device_type} {'✅' if device_type else '❌'}")
    logger.info(f"  date_from: {date_from} {'✅' if date_from else '❌'}")
    logger.info(f"  date_to: {date_to} {'✅' if date_to else '❌'}")
    logger.info(f"  host_id: {host_id} {'✅' if host_id else '❌'}")
    logger.info(f"  export_format: {export_format} {'✅' if export_format else '❌'}")
    logger.info("=" * 80)
    ...
    if not all([export_type, device_type, date_from, date_to, host_id]):
        ...
        missing_params = []
        if not export_type:
            missing_params.append("тип экспорта")
        if not device_type:
            missing_params.append("тип устройства")
        if not date_from:
            missing_params.append("дата начала")
        if not date_to:
            missing_params.append("дата окончания")
        if not host_id:
            missing_params.append("ID сайта")
        
        await callback.message.edit_text(
            f"❌ <b>Ошибка: Не все параметры заполнены</b>\n\n"
            f"<b>Отсутствуют:</b> {', '.join(missing_params)}\n\n"
            f"Пожалуйста, начните заново с выбора типа экспорта.",
    ...
    type_names = {
        ...
        "analytics": "🔬 Детальная аналитика",
        "enhanced": "🚀 Расширенный экспорт",
        "pages_in_search": "🔗 Страницы в поиске",
        "page_events": "📋 События со страницами"
    }
